...
static func apply(world_env: WorldEnvironment, theme: String) -> void:
	...
	env.fog_aerial_perspective = 0.55 if theme == "city" else 0.38
	env.fog_sky_affect = 0.82
	env.fog_depth_begin = 35.0
	env.fog_depth_end = 220.0 if theme == "city" else 280.0
...
static func _make_sky(theme: String) -> Sky:
	...
	match theme:
		"racing":
			...
			sky_mat.sky_horizon_color = Color(0.52, 0.72, 0.58)
			sky_mat.ground_horizon_color = Color(0.28, 0.5, 0.34)
			sky_mat.ground_bottom_color = Color(0.2, 0.42, 0.24)
		"city":
